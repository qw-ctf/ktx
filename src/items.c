...
gedict_t* Spawn_OnePoint(gedict_t *spawn_point, vec3_t org, int effects)
{
	...
	if (strcmp(color_tint, "1.0 1.0 1.0 1.0") != 0) {
		intptr_t nargs;

		trap_CmdTokenize(color_tint);
		nargs = trap_CmdArgc();
		if (nargs >= 3) {
			intptr_t colormod_ref;
			char argument[128];
			float colormod[3] = {0.0f, 0.0f, 0.0f};

			trap_CmdArgv(0, argument, sizeof(argument));
			colormod[0] = atof(argument);
			trap_CmdArgv(1, argument, sizeof(argument));
			colormod[1] = atof(argument);
			trap_CmdArgv(2, argument, sizeof(argument));
			colormod[2] = atof(argument);

			colormod_ref = trap_MapExtFieldPtr("colormod");
			trap_SetExtFieldPtr(p, colormod_ref, (intptr_t *) colormod, sizeof(colormod));
			...
			if (nargs == 4) {
				...
				alpha = bound(0.0f, atof(argument), 1.0f);
				alpha_ref = trap_MapExtFieldPtr("alpha");
				trap_SetExtFieldPtr(p, alpha_ref, (intptr_t *) &alpha, sizeof(alpha));
			}
		}
	}
