...
static void fb_spawn_button(gedict_t *ent)
{
	...
	// Only CTF routes bots to shoot buttons (via .bot LOOK_BUTTON paths), so only
	// there does a shootable button (health > 0) need to be part of the routing
	// graph. Outside CTF, keep the pre-bot-branch behaviour of leaving it out,
	// otherwise legacy .bot files gain dead-end paths into a marker that can only
	// be "reached" by firing at it.
	if (ent->s.v.health && !isCTF())
	{
		return;
	}
