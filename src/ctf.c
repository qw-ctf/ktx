...
void FlagThink(void)
{
	...
	if (self->cnt == FLAG_DROPPED)
	{
		...
		{
#ifdef BOT_SUPPORT
			// item_taken is only wired up when the flag spawned with bots enabled;
			// k_fb_enabled can be toggled on mid-map, so null-check before calling.
			if (bots_enabled() && self->fb.item_taken)
			{
