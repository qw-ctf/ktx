...
void TeamplayEventItemTaken(gedict_t *client, gedict_t *item)
{
	...
	{
		// tp_flags already holds which rune (it_rune1..4); it is set wherever the
		// rune entity is created (runes.c), so don't re-derive it from ctf_flag.
		client->tp.took.item = item->tp_flags;
	}
	else
	{
		return;
...
static char* CTFItemText(gedict_t *client)
{
	...
	if (HAVE_FLAG(client))
	{
		...
		// Separator only when a rune follows, otherwise callers that append their
		// own trailing space would produce a double space after a lone FLAG.
		if (HAVE_RUNE(client))
		{
