...
static float goal_flag_enemy(gedict_t *player, gedict_t *flag)
{
	...
	else if (flag->cnt == FLAG_AT_BASE)
	{
		...
		{
			float distance = VectorDistance(flag->s.v.origin, teammate->s.v.origin);
...
static float goal_rune(gedict_t *self, gedict_t *rune)
{
	...
	{
		// If a human is near and no enemies are, let them take the rune. The
		// teammate is already known not to carry a rune (checked just above).
		float distance = VectorDistance(rune->s.v.origin, teammate->s.v.origin);
		if (distance < 400 && (self->fb.enemy_dist >= 1500 || self->fb.enemy_dist == FB_NO_ENEMY_DIST))
		{
			return 0;
		}
	}
