...
void BotPerformHook(gedict_t *self)
{
	...
	// Note: lava-jump stepping is handled once per frame by BotPerformRocketJump,
	// which BotSetCommand always calls just before us under the same guard. Don't
	// call BotPerformLavaJump here or the state machine would advance twice a frame.
	...
	if (self->fb.hooking)
	{
		...
		distanceToTarget = VectorDistance(self->fb.hookTarget->s.v.origin, self->s.v.origin);
